...
# Then, we'll convert the centroid to ECEF space.
cam_position = GeoMakie.Geodesy.ECEFfromLLA(GeoMakie.wgs84)(
    GeoMakie.Geodesy.LLA( 
        sc_centroid[2], # latitude first to the LLA constructor
        sc_centroid[1], # longitude second
        2e7             # altitude third
    )
)
## Finally, we'll set the camera to look at the centroid.
## You can reuse `cc` after this to animate a camera moving, if you wish.
cc = cameracontrols(a.scene)
cc.eyeposition[] = cam_position
cc.lookat[] = [0.0, 0.0, 0.0]
cc.upvector[] = [0.0, 0.0, 1.0]
update_cam!(a.scene, cc)
f
...
cam_controller = lift(satellite_graph.satellite_position) do ecef
