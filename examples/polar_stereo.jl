...
Makie `PolarAxis` (`θ = lon`, `r = radial(lat)`) to get the **circular clip**, the **polar
...
disk is clipped at the cap, leaving the corners empty — cartopy's `always_circular_stereo` look.
...
`surface!`/`heatmap!` (drawn as a pcolormesh — raster, so prefer PNG). A field defined on the
