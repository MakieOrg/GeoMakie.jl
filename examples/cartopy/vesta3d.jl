...
# This example shows how to plot a 2D image of Vesta's surface in a 3D globe.
# It uses the `GlobeAxis` type to create a globe axis and plot the image on it.
# The ellipsoid parameters are set to the correct values for Vesta, which is a pretty
# oblate ellipsoid.
...
# Now that we have the data, we can go ahead and plot it in a GlobeAxis
# with the right ellipsoid params!
f, a, p = meshimage(
