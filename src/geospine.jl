...
function geospine_obs(ga::GeoAxis; padding = 10, density = 2)
    ...
    lift(ga.source_projection, ga.target_projection, ga.scene.px_area, ga.finallimits) do src, tgt, pxarea, finallims
...
"""
function get_finite_mask_of_projection(scene, transform_func, pxarea, finallims; padding = 10, density = 2, display = false)
    ...
    display && Base.display(heatmap(xs, ys, finite_mask; axis = (; aspect = DataAspect(),)))
...
get_finite_mask_of_projection(ga::GeoAxis; padding = 10, density = 2, display = false) = get_finite_mask_of_projection(ga.scene, create_transform(ga.target_projection[], ga.source_projection[]), ga.scene.px_area[], ga.finallimits[]; padding, density, display)

# get_finite_mask_of_projection(ga; display = true)

# fig = Figure(); ga = GeoAxis(fig[1, 1]; target_projection = "+proj=ortho"); lines!(ga, GeoMakie.coastlines()); fig
# geospineline = geospine_obs(ga)
# fig
# lines!(ga.scene, geospineline; color = :red, linewidth = 3, inspectable = false, xautolimits = false, yautolimits = false)
# lines(geospineline; axis = (;type = Axis, aspect = DataAspect()))
# fig

# save("hi.png", fig)

# @eval Main using GLMakie
# @eval Main CairoMakie.activate!(px_per_unit=3)
